...
description: "手动介入式视频下载：用有头浏览器（用户可见、不自动关闭）打开视频页，你手动完成登录、验证码等交互，工具在旁拦截真实流地址并多路并行高速下载，无损合并为 MP4，验收通过后再清理分片。适用于需要登录、短信验证码、滑动验证、观看密码或切换清晰度的受限视频站点（优酷、腾讯视频、爱奇艺、B 站等）；公开视频请直接用 yt-dlp。关键词：下载视频、视频下载、优酷下载、需要登录的视频、需要密码的视频、m3u8、ts 分片合并"
version: "1.1.1"
author: "CodeBuddy AI"
created: "2026-09-06"
updated: "2026-09-07"
...
| 场景 | 是否用本 Skill |
|---|---|
| 需要登录 / 验证码 / 滑动验证才能播放 | ✅ 正是本 Skill 的目标 |
| 需要会员才能看高画质 | ✅ |
| 需要作者设置的观看密码 | ✅ |
| 公开视频，yt-dlp / you-get 能直接下 | ❌ 直接用 yt-dlp，别折腾 |
